...
namespace uGE {
	...
	LuaHandler * SceneManager::_lua;
	glm::vec3 SceneManager::oldPosition;
    ...
    void SceneManager::add( LuaHandler * lua )
	{
		_lua = lua;
	}
	...
	void SceneManager::render( sf::Window * window )
	{
        ...
        //_shader->use(); // select the shader to use
            ...
            for ( auto i = _objects.begin(); i != _objects.end(); ++i )
            {
                ...
                shaderToUse->use();
                shaderToUse->setUniform(shaderToUse->time,Time::now());//set the current time in the shader as a float
	...
	void SceneManager::update()
	{
		...
		Time::update();
		if( _player )
            oldPosition = _player->getPosition();
