...
namespace uGE {
	...
	:	_program( program )
	{
		...
		index               = glGetUniformLocation( _program, "index" );
	}
...
//		GLuint name = mesh->getName();
	...
	void Shader::setTexture( GLuint location, GLuint index, Texture * texture )
	{
	    //activate the right slot trough the index
		glActiveTexture( GL_TEXTURE0 + index ); // units numbered as uniform locations with slot number
			//bind the texture id trough the index slot
			glBindTexture( GL_TEXTURE_2D, texture->getName() );
			//inform the shader which index is bound
